...
function makeDevServerV5Compatible(devServerConfig) {
  ...
  const existingSetupMiddlewares = compatibleConfig.setupMiddlewares;
  compatibleConfig.setupMiddlewares = (middlewares, devServer) => {
    const configuredMiddlewares = existingSetupMiddlewares
    ...
    configuredMiddlewares.unshift({
      ...
      middleware: (req, _res, next) => {
        ...
        if (req.method === "GET" && acceptsHtml && !isAsset && !isHealthRoute) {
          req.url = "/index.html";
        }
        next();
      },
